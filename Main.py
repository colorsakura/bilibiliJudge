...
def main():
    ...
    while True:
        ...
        operation, operation_print = VoteCalculate(
            voteBreak, voteDelete, voteRule, GiveUpEnable, JudgeProportion)
        if casestatus == 4:
            print('当前案件已裁决完毕，即将进行下一案件的审理……')
            continue
        while True:
            if(operation == 'CannotJudge'):
                print('目前案件{}的投票数量不足以判定操作，将在{}秒钟后重试！'.format(
                    caseinfo['data']['id'], delay))
                cannotJudge = True
                time.sleep(delay)
                voteBreak, voteDelete, voteRule, caseinfo, caseStatus = GetAndCal(
                    cid)
                operation, operation_print = VoteCalculate(
                    voteBreak, voteDelete, voteRule, GiveUpEnable, JudgeProportion)
            else:
        ...
        operation_output = '案件{}的投票结果计算为{}，正在进行投票操作……'.format(
            caseinfo['data']['id'], operation_print)
